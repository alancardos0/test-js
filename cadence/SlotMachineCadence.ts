...
function slotCadence(symbols: Array<SlotCoordinate>): SlotCadence {
  ...
  const getTotalColumns = function (index) {
    let valor = 0;
    valor += (result[index - 1] + currentCadence);
    ...
    if (totalSimbols >= anticipatorConfig.minToAnticipate) {
      currentCadence = anticipatorConfig.anticipateCadence;
    }
    if (totalSimbols >= anticipatorConfig.maxToAnticipate) {
      currentCadence = anticipatorConfig.defaultCadence;
    }
  };
  for (var index = 1; index < anticipatorConfig.columnSize; index++) {
    getTotalColumns(index);
  }
  // Magic
  return result;
}
